...
@Deprecated
public class LoggerReader implements Runnable {
	...
	public LoggerReader(String logPath, String packageName, String outputPath, HashSet<String> excludedmethods) {
	...
	private static List<File> filterByFileNames(File[] files) {
		...
		log(String.valueOf(files.length));
		for (File file : files) {
		...
		log(String.valueOf(results.size()));
		return results;
	}
	...
	public void process(String packageName, String outputPath) throws IOException {
		...
		if (new File(this.logPath).exists()) {
			...
			for (File file : files) {
				log("Parsing: " + packageName + ":" + file.getAbsolutePath());
	...
	private ArrayList<HashMap<String, String>> parseFile(File file, String packageMethodName) throws IOException {
		...
		// if (year.equals("2014")){
		log(String.format("Parsing file %s \t date(%s:%s:%s)  Package: %s ", date, day, month, year, packageMethodName,
				file.getTotalSpace()));
			...
			while (line != null) {
				...
				if (line != null) {
					...
					if (line.contains(packageMethodName)) {
						/**
						 * 11:59:36,209 INFO
						 * [ispyb.server.data.ejb3.services.saxs.ToolsForAssemblyWebService]
						 * [BIOSAXS_WS, storeDataAnalysisResultByMeasurementId,
						 * START, 1364896776209, -1, , ] 11:59:36,284 INFO
						 * [ispyb.server.data.ejb3.services.saxs.ToolsForAssemblyWebService]
						 * [BIOSAXS_WS, storeDataAnalysisResultByMeasurementId,
						 * END, 1364896776209, 75, ,
						 **/
						...
						if (type.trim().equals("END") || (type.trim().equals("ERROR"))) {
							if (!this.excludedmethods.contains(methodName)) {
								if (type.trim().equals("ERROR")) {
									String cause = "Unkwon";
									if (Arrays.asList(Arrays.asList(content.split("\\[")).get(0).split(","))
											.size() > 4) {
										cause = (Arrays.asList(Arrays.asList(content.split("\\[")).get(0).split(","))
												.get(5));
									}
									String error = "Unkwon";
								...
								this.excludedmethodsInformation.put(methodName,
										this.excludedmethodsInformation.get(methodName) + 1);
							}
						}
						// }
					}
	...
	private HashMap<String, String> getLogonRow(String methodName, String time, String duration, String user,
			String proposalType) {
	...
	private static HashMap<String, String> getRow(String methodName, String time, String duration, String message,
			String cause, String params) {
		SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss,SSS", Locale.FRANCE);
		HashMap<String, String> keys = new HashMap<String, String>();
		keys.put("time", sdf.format(new Date(Long.parseLong(time))));
		keys.put("method", methodName);
